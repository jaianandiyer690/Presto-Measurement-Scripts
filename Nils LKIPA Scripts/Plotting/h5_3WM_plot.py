...
for i in range(start_num, num_curves, step):
    ...
    S_11_sig_data = f[str(meas_num)]['Signal_Data'][()]
    S_11_idl_data = f[str(meas_num)]['Idler_Data'][()]
    freq = f[str(meas_num)]['Frequency'][()]

    S_11_sig_data_corr = correct_delay(S_11_sig_data, freq, delay)
    S_11_idl_data_corr = correct_delay(S_11_idl_data, freq, delay)

    pump_amp = f[str(meas_num)].attrs["v_pump"]
    legend.append(f"Pump_amp: {pump_amp}")
    # legend.append(f"Idler: {pump_amp}")

    if i == start_num:
        phi = -np.angle(S_11_sig_data_corr[0])

    S_11_sig_data_corr = correct_shift(S_11_sig_data_corr, phi)
    S_11_idl_data_corr = correct_shift(S_11_idl_data_corr, phi)

    S_11_sig_data_corr_rel = S_11_sig_data_corr
    S_11_idl_data_corr_rel = S_11_idl_data_corr
    ...
    # if ReIm:
    #     ax[0,0].plot(freq, 10*np.log10(np.abs(S_11_sig_data_corr_rel)), color = color)
    #     ax[0,1].plot(freq, np.angle(S_11_sig_data_corr_rel), color = color)

    #     ax[0,0].plot(freq, 10*np.log10(np.abs(S_11_idl_data_corr_rel)), color = color)
    #     ax[0,1].plot(freq, np.angle(S_11_idl_data_corr_rel), color = color)

    #     ax[1,0].plot(freq, np.real(S_11_sig_data_corr_rel), color = color,)
    #     ax[1,1].plot(freq, np.imag(S_11_sig_data_corr_rel), color = color)
    ...
    # else:
    #     ax[0].plot(freq, 10*np.log10(np.abs(S_11_sig_data_corr_rel)), color = color)
    #     ax[1].plot(freq, np.angle(S_11_sig_data_corr_rel), color = color)

    #     ax[0].plot(freq, 10*np.log10(np.abs(S_11_idl_data_corr_rel)), color = color)
    #     ax[1].plot(freq, np.angle(S_11_idl_data_corr_rel), color = color)
    ...
    ax[0].plot(freq, 10*np.log10(np.abs(S_11_sig_data_corr_rel)), color = color)
    ax[1].plot(freq, 10*np.log10(np.abs(S_11_idl_data_corr_rel)), color = color)
...
if save_fig:
    file_name = os.path.join(parent_folder_path, fab_name, meas, meas, png_title)
    fig.savefig(file_name)
    print(f"Saving PNG: {file_name + ".png"}")
